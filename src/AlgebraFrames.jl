...
import Base: length, (:), getindex, vect, show, pairs, filter!
...
function getindex(vec::Vector{RowElement}, s::String)
    vec[findfirst(row::RowElement -> row.name == s, vec)].value
end

function filter!(f::Function, af::AlgebraFrame{<:Any})
    af:vec -> begin
        mask = BitArray([begin
            f([RowElement(namms[e], val) for (e, val) in enumerate(row)])
        end for row in eachrow(vec)])
        vec = vec[mask]
    end
end
